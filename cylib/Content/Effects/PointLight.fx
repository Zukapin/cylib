...
PS_OUT PS( PS_IN input )
{
	...
	float4 temp = float4(input.tex * 2 - 1, depth.Sample(mySampler, input.tex).x, 1);
	temp.y = -temp.y;
	temp = mul(invViewProj, temp);
	float3 worldPos = temp.xyz / temp.w;
	float3 pointNormal = normalize(decode(normal.Sample(mySampler, input.tex).rg));

	float3 dirToLight = mul(viewMatrix, LightPosRadius.xyz - worldPos) / LightPosRadius.a;
	float atten = saturate(1 - length(dirToLight));
	float intensity = saturate(dot(pointNormal, normalize(dirToLight)));
	output.color.rgb = LightColorIntensity.xyz * intensity * atten * LightColorIntensity.w;
	output.color.a = 1;

	//output.color.rgb = 0;
	//output.color.r = ((worldPos.x + 10.0) / 20.0) * LightColorIntensity.r;
	//output.color.g = (1 - (worldPos.x + 10.0) / 20.0) * LightColorIntensity.g;
